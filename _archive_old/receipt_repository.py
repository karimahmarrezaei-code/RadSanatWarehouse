...
class ReceiptRepository:
    ...
    def get_pallets_for_reference(self, inbound_load_id: int) -> List[Dict[str, Any]]:
        ...
        (فیلتر بر اساس سابقه حذف شد: قبلاً فقط پالت‌هایی که در رسیدهای قبلی
        این مرجع بار بودند نمایش داده می‌شدند → بقیه پالت‌ها از کامبو حذف
        می‌شدند. حالا همه پالت‌های فعال همیشه در دسترس‌اند.)
        """
...
<head>
    ...
    <style>
        ...
        .header {{ display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; border-bottom: 2px solid #9333ea; padding-bottom: 15px; }}
        .company-info {{ flex: 1; }}
        .company-name {{ font-size: 24px; font-weight: bold; color: #000; margin-bottom: 5px; }}
        .company-info p {{ margin: 4px 0; color: #475569; font-size: 12px; }}
        .invoice-box {{
            background-color: #f3e5f5; border: 1px solid #ce93d8; padding: 10px;
            text-align: center; border-radius: 5px; min-width: 220px;
        }}
        .invoice-box h2 {{ color: #8e24aa; margin: 0 0 10px 0; font-size: 18px; }}
...
<div class="container">
        ...
        <div class="invoice-box">
            ...
            <div class="info-row"><strong>شماره رسید:</strong> {receipt_no}</div>
            <div class="info-row"><strong>شماره مرجع:</strong> {reference_no}</div>
            <div class="info-row"><strong>تاریخ:</strong> {operation_date_jalali}</div>
            <div class="info-row"><strong>مرحله:</strong> {stage_no}</div>
        </div>
    </div>
