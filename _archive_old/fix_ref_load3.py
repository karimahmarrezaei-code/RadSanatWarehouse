# -*- coding: utf-8 -*-
"""شرط مرجع باز = فقط مانده>0 - اجرا: python fix_ref_load3.py"""
...
for rel in ('app/ui/issue_manager_window.py', 'app/ui/combo_refresh_patch.py'):
    ...
    for i, l in enumerate(lines):
        if 'FROM outbound_loads' in l and any('remaining_qty' in lines[k] for k in range(i, min(len(lines), i + 7))):
            # افزودن remaining_qty به SELECT
            for k in range(max(0, i - 6), i + 1):
                if 'driver_id,' in lines[k] and 'remaining_qty' not in lines[k]:
                    lines[k] = lines[k].replace('driver_id,', 'driver_id, remaining_qty,', 1)
                    break
            # جایگزینی شرط WHERE تا قبل از ORDER BY
            j = i
            while j < len(lines) and 'ORDER BY id DESC' not in lines[j]:
                j += 1
            if j < len(lines):
                ind = len(lines[i + 1]) - len(lines[i + 1].lstrip())
                lines[i + 1:j] = [' ' * ind + '"WHERE is_active = 1 AND remaining_qty > 0 "']
                done = True
            break
    if done:
