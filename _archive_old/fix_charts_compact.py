# -*- coding: utf-8 -*-
"""هم‌قد کردن چارت‌ها با کارت‌ها - اجرا: python fix_charts_compact.py"""
...
n = 0
for nm in ('finance_chart', 'warehouse_chart', 'treasury_chart', 'activity_chart'):
    for old_h in ('150', '180'):
        old = f'self.{nm}.setMaximumHeight({old_h})'
        if old in s:
            s = s.replace(old, f'self.{nm}.setMinimumHeight(90); self.{nm}.setMaximumHeight(110)')
            n += 1
            break
print('چارت‌های پچ‌شده:', n)
if n == 0:
    print('⚠ هیچ کدام پیدا نشد')
...
print('تمام ✔')
print('=== تست 1366x768: چارت‌ها هم‌قد کارت‌ها ===')
input('Enter...')