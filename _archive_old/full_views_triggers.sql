CREATE VIEW IF NOT EXISTS v_combined_issues AS
    SELECT
      ...
      (SELECT COALESCE(SUM(wii.total_price),0) FROM warehouse_issue_items wii WHERE wii.issue_id=wi.id) AS amount,
      (SELECT COALESCE(SUM(pe.amount),0) FROM payment_entries pe JOIN financial_documents fd ON fd.id=pe.financial_document_id WHERE fd.issue_id=wi.id) AS paid_amount,
