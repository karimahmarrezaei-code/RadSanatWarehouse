# -*- coding: utf-8 -*-
"""
update_autocomplete_v4.py - رفع قطعی F2 (نسخه ۳)
==================================================
...
اجرا (از پوشه F:\\warehouse_app — برنامه بسته باشد):
    py -X utf8 .\\update_autocomplete_v4.py
...
def install_autocomplete(window):
    ...
    try:
        ...
        print('[auto-complete] installed on {}: {} QLineEdit + {} QTextEdit, F2 ready'.format(
...
def update_core():
    ...
    log('   ✅ auto_complete.py نسخه ۴ — سینتکس: ' + ('سالم' if ok else '❌ ' + str(err)[:100]))
    return ok
...
def main():
    log('=== به‌روزرسانی auto-complete (نسخه ۴: رفع isPassword (سازگار با PyQt5 قدیمی)) ===')
