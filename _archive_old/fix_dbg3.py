# -*- coding: utf-8 -*-
"""شاهد با تورفتگی هم‌تراز - اجرا: python fix_dbg3.py"""
...
W = ("open(_o9.path.join(_o9.path.dirname(_s9.executable), 'STOCK_DEBUG.txt'), 'a', encoding='utf-8')"
     ".write(%s).close()  # DBG")
...
for rel in ('app/ui/proforma_window.py', 'app/ui/ui/proforma_window.py'):
    ...
    for i, l in enumerate(lines):
        if l.strip().startswith('def _apply_reserved_stocks'):
            ...
            lines[i + 1:i + 1] = blk(ind, ["import sys as _s9, os as _o9  # DBG", W % "'ENTER\\n'"])
            n += 1
            break
    for i, l in enumerate(lines):
        if 'physical = free_stock_map(self.db)  # ONE-STOCK' in l:
            ind = len(l) - len(l.lstrip())
            lines[i + 1:i + 1] = blk(ind, ["import sys as _s9, os as _o9  # DBG", W % "'physical=' + repr(physical) + '\\n'"])
            n += 1
            break
    for i, l in enumerate(lines):
        if 'stocks = {pid: max(int(physical.get(pid, 0))' in l:
            ind = len(l) - len(l.lstrip())
            lines[i + 1:i + 1] = blk(ind, ["import sys as _s9, os as _o9  # DBG", W % "'stocks=' + repr(stocks) + '\\n'"])
            n += 1
            break
    for i, l in enumerate(lines):
        if 'مانده انبار: {:,}' in l:
            ind = len(l) - len(l.lstrip())
            lines[i + 1:i + 1] = blk(ind, ["import sys as _s9, os as _o9  # DBG", W % "'txt=' + repr(txt) + '\\n'"])
            n += 1
            break
    if n:
