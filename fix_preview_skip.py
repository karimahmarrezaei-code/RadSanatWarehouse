# -*- coding: utf-8 -*-
"""معافیت پیش‌نمایش از پیچ اسکرول - اجرا: python fix_preview_skip.py"""
import os, sys, subprocess, shutil, py_compile
ROOT = os.path.dirname(os.path.abspath(__file__)); os.chdir(ROOT)

mp = os.path.join(ROOT, 'main.py'); s = open(mp, encoding='utf-8').read()
if 'QWebEngineView as _QWV' not in s:
    anchor = 'def _safe_fit(w):\n'
    if anchor in s:
        s = s.replace(anchor, anchor +
                      "    try:  # PREV-SKIP\n"
                      "        from PyQt5.QtWebEngineWidgets import QWebEngineView as _QWV  # PREV-SKIP\n"
                      "        if w.findChildren(_QWV):  # PREV-SKIP\n"
                      "            return  # PREV-SKIP\n"
                      "    except Exception:  # PREV-SKIP\n"
                      "        pass  # PREV-SKIP\n", 1)
        open(mp, 'w', encoding='utf-8').write(s)
        py_compile.compile(mp, doraise=True)
        print('1) معافیت نصب شد ✔')
    else:
        print('1) ⚠ لنگر پیدا نشد')
else:
    print('1) از قبل بود ✔')
