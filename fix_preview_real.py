# -*- coding: utf-8 -*-
"""بازنویسی ماژول واقعی پیش‌نمایش - اجرا: python fix_preview_real.py"""
...
for rel in ('app/ui/html_preview_dialog.py', 'app/ui/ui/html_preview_dialog.py'):
    ...
    m = re.search(r'^class (\w+)\s*\(', old, re.M)
    name = m.group(1) if m else 'HtmlPreviewDialog'
    open(p, 'w', encoding='utf-8').write(TPL.replace('{NAME}', name))
    py_compile.compile(p, doraise=True)
    print('1)', rel, '-> کلاس:', name, '✔')
