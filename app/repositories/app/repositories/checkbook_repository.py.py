# -*- coding: utf-8 -*-
"""دفترچه چک - ریپازیتوری خودکفا (جداول checkbook_*)"""
...
class CheckbookRepository:
    ...
    # 👇 همین بلوک را اضافه کنید (داخل کلاس)
    def import_from_finance(self, user_id: int) -> int:
        """کپی چک‌های دریافتنیِ در انتظار از ماژول مالی به دفترچه (بدون تکرار)"""
        ...
        for r in rows:
            ...
            with self.db.connect() as conn:
                exists = conn.execute(
                    "SELECT 1 FROM checkbook_checks WHERE source_type='FINANCE' AND source_id=?",
                    (r['id'],)).fetchone()
            if exists:
                continue
            self.create_check({
    ...
    def _move(self, cid, move_type, to_person, ref_type, ref_id, note, user_id):
        ...
        with self.db.connect() as conn:
            conn.execute(
                "INSERT INTO checkbook_movements (check_id,move_type,move_date,from_person_id,"
