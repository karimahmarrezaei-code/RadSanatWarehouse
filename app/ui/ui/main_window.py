...
class SummaryCard(QFrame):
    def __init__(self, title: str, value: str, color: str = "#2563eb") -> None:
        ...
        self.setStyleSheet(f"QFrame#Card {{  border:2px solid {color}; border-radius:8px; padding:12px; min-height:110px; }}")
        layout = QVBoxLayout(self); layout.setSpacing(6); layout.setContentsMargins(10, 8, 10, 8)
        t = QLabel(title); t.setObjectName('Muted'); t.setAlignment(Qt.AlignCenter); t.setWordWrap(True)
        v = QLabel(value); v.setStyleSheet(f"QLabel{{color:{color};font-size:28px;font-weight:bold;background:transparent;border:none;padding:0;}}"); v.setAlignment(Qt.AlignCenter)
...
class ChartBrowser(QTextBrowser):
    def __init__(self) -> None:
        ...
        self.setMinimumHeight(150); self.setMaximumHeight(200)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.cards_layout.setSpacing(16); self.cards_layout.setContentsMargins(6, 6, 6, 6)
        main_layout.addWidget(cards_container)

        charts = QWidget(); cl = QVBoxLayout(charts); cl.setSpacing(8); cl.setContentsMargins(0, 0, 0, 0)
        top = QHBoxLayout(); top.setSpacing(8)
        self.finance_chart = ChartBrowser(); self.finance_chart.setMaximumHeight(180)
        self.warehouse_chart = ChartBrowser(); self.warehouse_chart.setMaximumHeight(180)
        top.addWidget(self.finance_chart, 1); top.addWidget(self.warehouse_chart, 1)
        bot = QHBoxLayout(); bot.setSpacing(8)
        self.treasury_chart = ChartBrowser(); self.treasury_chart.setMaximumHeight(180)
        self.activity_chart = ChartBrowser(); self.activity_chart.setMaximumHeight(180)
