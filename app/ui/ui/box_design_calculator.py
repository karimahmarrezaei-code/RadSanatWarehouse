"""
ابزار محاسبه‌گر طراحی جعبه چوبی - نسخه کاملاً تمیز
بدون هیچ print statement - فقط QMessageBox برای پیام‌ها
"""
...
MATERIAL_ITEMS = [
    ...
    {'type': 'labor_per_hour', 'name': 'دستمزد ساعتی', 'default_price': 200000, 'unit': 'ریال/ساعت'},
...
class BoxDesignCalculator(QDialog):
    ...
    def _build_ui(self):
        ...
        subtitle = QLabel('ابعاد را وارد کنید - محاسبه ابعاد، نقشه فنی سه‌نما و قیمت نهایی به صورت خودکار')
        subtitle.setObjectName('Muted')
        h_layout.addWidget(title)
        h_layout.addWidget(subtitle)
        root.addWidget(header)

        input_group = QGroupBox('مشخصات جعبه')
        ...
        dims_layout.addWidget(QLabel('طول:')); dims_layout.addWidget(self.length_input)
        dims_layout.addWidget(QLabel('عرض:')); dims_layout.addWidget(self.width_input)
        dims_layout.addWidget(QLabel('ارتفاع:')); dims_layout.addWidget(self.height_input)
        input_layout.addRow('ابعاد (cm):', dims_layout)
        ...
        weight_layout.addWidget(QLabel('وزن (kg):')); weight_layout.addWidget(self.weight_input)
        ...
        count_layout.addWidget(QLabel('تعداد:')); count_layout.addWidget(self.count_input)
        ...
        print_btn = QPushButton('چاپ نقشه فنی')
        ...
        save_btn = QPushButton('ثبت تولید در انبار')
        ...
        prices_info = QLabel('قیمت‌ها به ازای هر کاربر ذخیره می‌شوند.')
    ...
    def _save_material_prices(self):
        ...
        if self.db and self.user_id:
            try:
                ...
                QMessageBox.warning(self, 'خطا در دیتابیس', f'خطا:\n{e}')
        
        self.material_prices = new_prices.copy()
        self._show_prices_in_table()
        
        QMessageBox.information(self, 'موفق', f'{len(new_prices)} قیمت ذخیره شد.')
    ...
    def _calculate(self):
        ...
        QMessageBox.information(self, 'موفق', 'محاسبه با موفقیت انجام شد!')
...
<div class="box">
    ...
    <table>
        ...
        <tr><td>ابعاد</td><td>{r['item_length']} x {r['item_width']} x {r['item_height']} cm</td></tr>
        <tr><td>وزن</td><td>{r['item_weight']} kg</td></tr>
        <tr><td>تعداد</td><td>{r['item_count']} عدد</td></tr>
        <tr><td>استقامت</td><td>{r['strength_key']}</td></tr>
        <tr><td>چوب</td><td>{r['wood_key']} (چگالی: {r['wood_density']} g/cm3)</td></tr>
    </table>
</div>
...
<div class="box cost">
    ...
    <table>
        ...
        <tr><td>2</td><td>یراق‌آلات</td><td>{self._format_price(r['hardware_cost'])}</td></tr>
        <tr><td>3</td><td>دستمزد ({r['labor_hours']} ساعت x {r['labor_workers']} کارگر x {self._format_price(r['labor_rate'])})</td><td>{self._format_price(r['labor_cost'])}</td></tr>
...
<text x="{total_w/2}" y="130" text-anchor="middle" font-size="15" font-weight="bold" fill="#46505f">ابعاد خارجی: {L:.1f} x {W:.1f} x {H:.1f} cm | چوب: {r['wood_key']} | ضخامت: {T:.1f} cm</text>
...
<g transform="translate(50, 520)">
    ...
    <text x="30" y="110" font-size="12" fill="#5b6675">دستمزد:</text>
...
<div class="dims">
    ابعاد خارجی: {L:.1f} x {W:.1f} x {H:.1f} cm | چوب: {r['wood_key']} | ضخامت: {T:.1f} cm
</div>
...
<table>
    <tr>
        ...
        <td class="view">
            ...
            <div class="dim dim-blue">{L:.1f} x {W:.1f} cm</div>
        </td>
    </tr>
</table>
...
<table class="price">
    ...
    <tr><td>دستمزد ({r['labor_hours']} ساعت x {r['labor_workers']} کارگر):</td><td>{self._format_price(r['labor_cost'])}</td></tr>
...
<div class="material">
    ...
    <table>
        ...
        <tr><td>2</td><td>پیچ چوب 4x40</td><td>{r['total_screws_4x40']} عدد</td><td>{self._format_price(self._get_price('screw_4x40'))}</td><td>{self._format_price(r['total_screws_4x40'] * self._get_price('screw_4x40'))}</td></tr>
        <tr><td>3</td><td>پیچ چوب 5x60</td><td>{r['total_screws_5x60']} عدد</td><td>{self._format_price(self._get_price('screw_5x60'))}</td><td>{self._format_price(r['total_screws_5x60'] * self._get_price('screw_5x60'))}</td></tr>
        ...
        <tr><td>11</td><td>دستمزد ({r['labor_hours']} ساعت x {r['labor_workers']} کارگر)</td><td>{r['labor_hours'] * r['labor_workers']} ساعت</td><td>{self._format_price(self._get_price('labor_per_hour'))}</td><td>{self._format_price(r['labor_cost'])}</td></tr>
...
<li>از داخل با پیچ‌های 4x40 محکم کنید</li>
</ul></div>
    ...
    def _print_result(self):
        ...
        try:
            ...
            QMessageBox.information(self, 'چاپ', 
                'نقشه فنی کامل SVG در مرورگر باز شد.\n\n'
                'برای چاپ: Ctrl+P را بزنید\n'
                f'فایل: {tmp.name}')
        except Exception as e:
        ...
        try:
            with self.db.connect() as conn:
                ...
                QMessageBox.information(self, 'موفق',
                    ...
                    f'کد جعبه: {box_code}\n'
                    f'ابعاد: {r["outer_length"]:.1f} x {r["outer_width"]:.1f} x {r["outer_height"]:.1f} cm\n'
